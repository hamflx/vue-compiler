...
- `cargo xtask run-option-matrix --all` now executes real official-vs-Rust npm alias probes and writes `target/conformance/<lock-hash>/option-matrix.json`; it currently fails for Vue 2.7 and Vue 3 targets, while Vue 2.6 has 6 passing executable rows and 4 explicit pending rows.
