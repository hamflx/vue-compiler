...
- `cargo xtask run-option-matrix --all` now executes real official-vs-Rust npm alias probes and writes `target/conformance/<lock-hash>/option-matrix.json`; Vue 2.7 and Vue 3 SFC targets now pass their current four-row matrices, while other target matrices still need closure.
