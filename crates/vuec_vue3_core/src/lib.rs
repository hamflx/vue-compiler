...
fn lower_vue3_static_slot_to_dom_mir(
    ast_id: NodeId,
    ast_node: &vuec_ast::Node<Vue3NodeKind>,
    slot: &Vue3Directive,
    children: &[NodeId],
...
fn lower_vue3_default_slot_to_dom_mir(
    ast_node: &vuec_ast::Node<Vue3NodeKind>,
...
fn lower_vue3_for_slot_to_dom_mir(
    ast_id: NodeId,
    ast_node: &vuec_ast::Node<Vue3NodeKind>,
    slot: &Vue3Directive,
...
fn lower_vue3_dynamic_slot_object_to_dom_mir(
    ast_id: NodeId,
    ast_node: &vuec_ast::Node<Vue3NodeKind>,
    slot: &Vue3Directive,
    children: &[NodeId],
...
fn lower_vue3_slot_name_to_dom_mir(
    slot: &Vue3Directive,
    ast_node: &vuec_ast::Node<Vue3NodeKind>,
    state: &mut Vue3DomLoweringState,
) -> Vue3DomSlotName {
...
fn vue3_slot_name_text(name: &Vue3DomSlotName) -> String {
    match name {
...
fn lower_vue3_slot_decl_to_hir(
    ast_id: NodeId,
    hir_parent: NodeId,
    mir_id: NodeId,
    name: String,
...
fn vue3_static_slot_directive_name(slot: &Vue3Directive) -> String {
    slot.arg
        .as_ref()
...
fn lower_vue3_slot_children_to_dom_mir(
    children: &[NodeId],
    ast: &Vue3Ast,
    hir_parent: NodeId,
    mir_parent: NodeId,
...
fn component_has_dynamic_slots(ast: &Vue3Ast, children: &[NodeId]) -> bool {
...
fn register_vue3_pattern_with_span(
    store: &mut JsAstStore,
    expression: &Vue3Expression,
    span: Option<Span>,
    source_type: oxc_span::SourceType,
...
impl<'a> Vue3SsrMirCodegen<'a> {
    fn new(mir: &'a Vue3SsrMir, js: &'a JsAstStore, options: &'a Vue3CompilerOptions) -> Self {
        Self { mir, js, options }
    }
    ...
    fn generate(self) -> CodegenResult {
        ...
        self.render_children(self.mir.root, &scope, &mut writer);
        if use_with {
            writer.dedent();
            writer.push_line("}");
        }
    ...
    fn render_function_start(&self, writer: &mut CodeWriter) {
        let args = "_ctx, _push, _parent, _attrs";
        if self.options.inline {
    ...
    fn render_children(&self, parent: NodeId, scope: &RenderScope, writer: &mut CodeWriter) {
        ...
        for child_id in &node.children {
            self.render_node(*child_id, scope, writer);
        }
    }

    fn render_node(&self, node_id: NodeId, scope: &RenderScope, writer: &mut CodeWriter) {
    ...
    fn render_slot(
        ...
        let fallback = if slot.fallback.is_empty() {
            ...
            for child_id in &slot.fallback {
                self.render_node(*child_id, scope, &mut fallback);
            }
            fallback.dedent();
            fallback.push_line("}");
    ...
    fn render_if(
        ...
        if let Some(alternate) = alternate {
            ...
            writer.indent();
            self.render_node(alternate, scope, writer);
            writer.dedent();
            writer.push_line("}");
        } else {
            writer.push_line("}");
...
fn escape_static_html_text(value: &str) -> String {
...
fn event_handler_prop_name_for_component(event: &str) -> String {
    let raw_name = event
        .strip_prefix("vue:")
...
fn event_handler_prop_name_for_element(event: &str) -> String {
    let raw_name = event
        .strip_prefix("vue:")
        .map(|hook| format!("vnode-{hook}"))
        .unwrap_or_else(|| event.to_string());
...
impl<'a> PublicAstCodegen<'a> {
    fn new(root: &'a Value, options: &'a Vue3CompilerOptions) -> Self {
        Self {
            root,
            options,
            code: String::new(),
    ...
    fn generate(mut self) -> CodegenResult {
        self.gen_preamble();
    ...
    fn gen_node(&mut self, node: &Value) {
        match node {
    ...
    fn gen_node_list(&mut self, nodes: &[Value], multilines: bool, comma: bool) {
    ...
    fn gen_node_list_as_array(&mut self, nodes: &[Value]) {
    ...
    fn gen_vnode_call(&mut self, node: &Value) {
    ...
    fn gen_call_expression(&mut self, node: &Value) {
    ...
    fn gen_object_expression(&mut self, node: &Value) {
        let properties = node
            .get("properties")
            .and_then(Value::as_array)
            .map(Vec::as_slice)
    ...
    fn gen_property_key(&mut self, key: &Value) {
    ...
    fn gen_function_expression(&mut self, node: &Value) {
    ...
    fn gen_conditional_expression(&mut self, node: &Value) {
    ...
    fn gen_cache_expression(&mut self, node: &Value) {
    ...
    fn gen_template_literal(&mut self, node: &Value) {
        self.push("`");
        let elements = node
            .get("elements")
            .and_then(Value::as_array)
    ...
    fn gen_if_statement(&mut self, node: &Value) {
        self.push("if (");
    ...
    fn gen_raw_string(&mut self, value: &str) {
...
fn public_helper_by_name(name: &str) -> Option<RuntimeHelper> {
    match name {
...
fn public_helper_aliases(helpers: &[RuntimeHelper]) -> String {
    helpers
        .iter()
        .map(|helper| format!("{}: _{}", helper_name(*helper), helper_name(*helper)))
        .collect::<Vec<_>>()
...
fn hoist_static_helpers(helpers: &[RuntimeHelper]) -> Vec<RuntimeHelper> {
    [
        RuntimeHelper::Vue3CreateVNode,
        RuntimeHelper::Vue3CreateElementVNode,
        RuntimeHelper::Vue3CreateCommentVNode,
...
fn is_ssr_helper(helper: RuntimeHelper) -> bool {
    matches!(
        helper,
        RuntimeHelper::Vue3SsrInterpolate
...
fn public_patch_flag_value(value: &Value) -> Value {
...
fn public_patch_flag_text(flag: i32) -> String {
...
fn patch_flag_names(flag: i32) -> Vec<&'static str> {
    if flag == -1 {
        return vec!["CACHED"];
    }
    if flag == -2 {
