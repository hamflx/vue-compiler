...
#[derive(Default)]
struct Vue3BindSuiteState {
    errors: Vec<Value>,
}

fn vue3_core_transform_bind_suite_value(payload: &Value) -> Value {
    let source = template_source(payload);
    let options = vue3_options(payload.get("options"));
    let browser = payload
        .get("options")
...
fn vue3_bind_suite_transform_node(
    mut node: Value,
...
fn vue3_bind_suite_apply_shorthand(
    node: &mut Value,
    browser: bool,
    state: &mut Vue3BindSuiteState,
) {
...
fn vue3_bind_suite_process_directive_expressions(node: &mut Value, options: &Vue3CompilerOptions) {
...
fn vue3_bind_suite_codegen(node: &Value, browser: bool, state: &mut Vue3BindSuiteState) -> Value {
...
fn vue3_bind_suite_materialize_projection(projection: Option<&Value>, dir: &Value) -> Value {
...
fn vue3_bind_suite_has_dynamic_key(node: &Value) -> bool {
...
fn vue3_bind_suite_property_key_is_dynamic(key: &Value) -> bool {
...
fn vue3_bind_suite_error_value(error: &Value, dir: &Value) -> Value {
    let code = error
        .as_u64()
...
fn vue3_bind_suite_transform_context(browser: bool) -> Value {
    json!({
        "inSSR": false,
        "browser": browser,
    })
...
fn vue3_bind_suite_helper_name(helper: &str) -> &str {
    match helper {
        "CAMELIZE" => "camelize",
...
#[derive(Default)]
struct Vue3ModelSuiteState {
    errors: Vec<Value>,
    cached: usize,
}

#[derive(Default)]
struct Vue3ForSuiteState {
    errors: Vec<Value>,
}

fn vue3_core_transform_for_suite_value(payload: &Value) -> Value {
    let source = template_source(payload);
    let options = vue3_options(payload.get("options"));
    let ast = Vue3Dialect::base_parse(source.clone(), &options);
    let mut root = vue3_parse_value(
...
fn vue3_for_suite_transform_children(
    children: Vec<Value>,
    options: &Vue3CompilerOptions,
...
fn vue3_for_suite_transform_node(
    mut node: Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ForSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_for_suite_transform_if_node(
    mut node: Value,
    siblings: &[Value],
    index: usize,
    options: &Vue3CompilerOptions,
...
fn vue3_for_suite_transform_for_node(
    mut node: Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ForSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_for_suite_apply_bind_shorthand(node: &mut Value, state: &mut Vue3ForSuiteState) {
    let projection = vuec_vue3_core::transform_v_bind_shorthand_projection(&json!({
        "node": node,
...
fn vue3_for_suite_process_expression_node(
    node: &mut Value,
    key: &str,
    options: &Vue3CompilerOptions,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_for_suite_element_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ForSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_for_suite_props_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ForSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_for_suite_push_props_object_arg(
...
fn vue3_for_suite_props_object(properties: Vec<Value>, node: &Value) -> Value {
    json!({
        "type": 15,
        "properties": properties,
        "loc": node.get("loc").cloned().unwrap_or_else(vue3_loc_stub_value),
...
fn vue3_for_suite_ref_for_property() -> Value {
...
fn vue3_for_suite_prepend_props_expression_prop(
    mut props: Value,
    property: Value,
    loc: Value,
) -> Value {
...
fn vue3_for_suite_prepend_object_property(object: &mut Value, property: Value) {
...
fn vue3_for_suite_if_codegen(if_node: &mut Value, key_base: usize) -> Value {
    let branch = if_node
        .get("branches")
        .and_then(Value::as_array)
...
fn vue3_for_suite_key_property(
...
fn vue3_for_suite_branch_key_property(index: usize) -> Value {
    vue3_once_suite_object_property(
        vue3_once_suite_simple_expression("key", true),
        json!({
            "type": 4,
...
fn vue3_for_suite_inject_prop(vnode_call: &mut Value, property: Value) {
...
fn vue3_for_suite_loop_params(parse_result: &Value) -> Vec<Value> {
...
fn vue3_for_suite_for_node_payload(for_node: &Value) -> Value {
    json!({
...
fn vue3_for_suite_slot_outlet_context(
    options: &Vue3CompilerOptions,
    scope: &Vue3ModelSuiteScope,
) -> Value {
...
fn vue3_for_suite_finalize_root(root: &mut Value) {
...
fn vue3_for_suite_helpers(root: &Value) -> Vec<String> {
...
fn vue3_for_suite_collect_helpers(node: &Value, used: &mut Vec<&'static str>) {
...
fn vue3_if_suite_transform_children(
    children: Vec<Value>,
    options: &Vue3CompilerOptions,
...
fn vue3_if_suite_transform_if_chain(
    siblings: &[Value],
    start: usize,
    key_base: usize,
    options: &Vue3CompilerOptions,
...
fn vue3_if_suite_branch_from_node(
    mut node: Value,
    dir_name: &str,
    comments: Vec<Value>,
    options: &Vue3CompilerOptions,
...
fn vue3_if_suite_transform_node(
    mut node: Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3IfSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_if_suite_element_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3IfSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_if_suite_props_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3IfSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_if_suite_branch_codegen(branch: &mut Value, key_index: usize) -> Value {
    let projection = vuec_vue3_core::transform_if_projection(&json!({
        "phase": "branchCodegen",
        "branch": branch,
        "keyIndex": key_index,
...
fn vue3_if_suite_inject_prop(target: &mut Value, property: Value) {
...
fn vue3_if_suite_inject_prop_into_props(props: Value, property: Value) -> Value {
    if props.is_null() || props.is_string() {
...
fn vue3_if_suite_push_props_object_arg(
    merge_args: &mut Vec<Value>,
    properties: &mut Vec<Value>,
    node: &Value,
) {
...
fn vue3_if_suite_props_object(properties: Vec<Value>, node: &Value) -> Value {
    json!({
        "type": 15,
        "properties": properties,
        "loc": node.get("loc").cloned().unwrap_or_else(vue3_loc_stub_value),
...
fn vue3_if_suite_apply_bind_shorthand(node: &mut Value, state: &mut Vue3IfSuiteState) {
    let projection = vuec_vue3_core::transform_v_bind_shorthand_projection(&json!({
        "node": node,
        "context": { "browser": false },
    }));
...
fn vue3_if_suite_user_key(node: &Value) -> Option<Value> {
    node.get("props")
        .and_then(Value::as_array)?
        .iter()
...
fn vue3_if_suite_same_user_key(a: &Value, b: &Value) -> bool {
...
fn vue3_if_suite_is_comment_or_ascii_whitespace(node: &Value) -> bool {
    match vue3_public_node_type(node) {
        Some(3) => true,
        Some(2) => node
            .get("content")
...
fn vue3_if_suite_collect_directives(root: &Value) -> Vec<String> {
    let mut directives = Vec::new();
    vue3_if_suite_collect_directives_for_node(root, &mut directives);
    directives
}

fn vue3_if_suite_collect_directives_for_node(node: &Value, directives: &mut Vec<String>) {
...
fn vue3_if_suite_helpers(root: &Value) -> Vec<String> {
    let mut used = Vec::new();
...
fn vue3_if_suite_collect_helpers(node: &Value, used: &mut Vec<&'static str>) {
    match vue3_public_node_type(node) {
        Some(9) => {
...
fn vue3_if_suite_collect_branch_child_helpers(branch: &Value, used: &mut Vec<&'static str>) {
...
fn vue3_if_suite_collect_call_helpers(node: &Value, used: &mut Vec<&'static str>) {
...
struct Vue3SlotSuiteState {
    ...
    transform_element_helpers: Vec<String>,
}
...
fn vue3_core_transform_element_suite_value(payload: &Value) -> Value {
    let source = template_source(payload);
    let options = vue3_options(payload.get("options"));
...
fn vue3_transform_element_suite_self_name(filename: &str) -> Option<String> {
...
fn vue3_transform_element_suite_apply_style(node: &mut Value) {
    if vue3_public_node_type(node) == Some(1) {
...
fn vue3_transform_element_suite_result_node(root: &Value) -> Value {
    root.get("children")
        .and_then(Value::as_array)
        .and_then(|children| children.first())
        .and_then(|wrapper| {
...
fn vue3_transform_element_suite_element_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3SlotSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_transform_element_suite_plain_element_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3SlotSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_transform_element_suite_component_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3SlotSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_transform_element_suite_props_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3SlotSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_transform_element_suite_runtime_directive(dir: &Value) -> Value {
...
fn vue3_transform_element_suite_directive_runtime(runtime: &Value, dir: &Value) -> Value {
...
fn vue3_transform_element_suite_apply_inline_template_refs(
    props: &mut Value,
    projection: &Value,
    node: &Value,
) {
...
fn vue3_transform_element_suite_apply_inline_template_ref_value(props: &mut Value, content: &str) {
    let Some(properties) = props.get_mut("properties").and_then(Value::as_array_mut) else {
        return;
    };
    for property in properties {
...
fn vue3_transform_element_suite_is_dynamic_component_is_prop(node: &Value, prop: &Value) -> bool {
    matches!(
        node.get("tag").and_then(Value::as_str),
        Some("component" | "Component")
...
fn vue3_transform_element_suite_static_arg(prop: &Value) -> Option<String> {
    prop.get("arg")
        .filter(|value| !value.is_null())
        .filter(|arg| {
            arg.get("isStatic")
...
fn vue3_transform_element_suite_has_modifier(prop: &Value, name: &str) -> bool {
    prop.get("modifiers")
        .and_then(Value::as_array)
...
fn vue3_transform_element_suite_dedupe_properties(properties: Vec<Value>) -> Vec<Value> {
...
fn vue3_transform_element_suite_merge_property_value(property: &mut Value, next_value: Value) {
...
fn vue3_transform_element_suite_cache_expression(
    state: &mut Vue3SlotSuiteState,
    value: Value,
    need_array_spread: bool,
) -> Value {
...
fn vue3_transform_element_suite_component_context(
    options: &Vue3CompilerOptions,
    state: &Vue3SlotSuiteState,
    scope: &Vue3ModelSuiteScope,
) -> Value {
...
fn vue3_transform_element_suite_component_tag(
    projection: &Value,
    node: &Value,
    state: &mut Vue3SlotSuiteState,
) -> Value {
...
fn vue3_transform_element_suite_register_projection_helpers(
    projection: &Value,
    state: &mut Vue3SlotSuiteState,
) {
    if projection
...
fn vue3_transform_element_suite_should_build_component_slots(
    tag: &Value,
    children: &Value,
) -> bool {
    if children.is_null() {
...
fn vue3_transform_element_suite_helper_tag_name(tag: &Value) -> Option<&str> {
    tag.as_str()
}

fn vue3_transform_element_suite_element_children(node: &Value) -> Value {
    let children = node
        .get("children")
        .and_then(Value::as_array)
        .cloned()
...
fn vue3_transform_element_suite_push_unique(items: &mut Vec<String>, value: String) {
...
fn vue3_transform_element_suite_known_helper(helper: &str) -> Option<&'static str> {
    match helper {
...
fn vue3_model_suite_transform_children(
    children: Vec<Value>,
    options: &Vue3CompilerOptions,
...
fn vue3_model_suite_transform_node(
    mut node: Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ModelSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_model_suite_transform_for_node(
    mut node: Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ModelSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_model_suite_process_directive_expressions(
    node: &mut Value,
    options: &Vue3CompilerOptions,
    scope: &Vue3ModelSuiteScope,
) {
...
fn vue3_model_suite_track_slot_scope(node: &Value, scope: &mut Vue3ModelSuiteScope) {
    if vue3_public_node_type(node) != Some(1) {
        return;
    }
    let projection = vuec_vue3_core::track_slot_scopes_projection(&json!({ "node": node }));
...
fn vue3_model_suite_add_locals(scope: &mut Vue3ModelSuiteScope, locals: Option<&Value>) {
    for local in locals
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
...
fn vue3_model_suite_element_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ModelSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_model_suite_props_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ModelSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_model_suite_materialize_projection(projection: Option<&Value>, dir: &Value) -> Value {
    let Some(projection) = projection else {
        return Value::Null;
    };
    if projection.is_string() || projection.get("type").is_some() {
...
fn vue3_model_suite_cache_expression(
    state: &mut Vue3ModelSuiteState,
    value: Value,
    need_pause_tracking: bool,
    in_v_once: bool,
...
fn vue3_model_suite_static_prop_name(key: &Value) -> Option<String> {
    (vue3_public_node_type(key) == Some(4)
        && key
            .get("isStatic")
            .and_then(Value::as_bool)
...
fn vue3_model_suite_dynamic_props_string(props: &[String]) -> String {
    let values = props
        .iter()
...
fn vue3_model_suite_finalize_root(root: &mut Value, state: &Vue3ModelSuiteState) {
    vue3_once_suite_set_root_codegen(root);
    root["components"] = json!(vue3_once_suite_components(root));
...
fn vue3_model_suite_helpers(root: &Value) -> Vec<String> {
    let mut used = Vec::new();
...
fn vue3_model_suite_collect_helpers(node: &Value, used: &mut Vec<&'static str>) {
...
fn vue3_model_suite_error_value(error: &Value, dir: &Value) -> Value {
    let code = error
        .as_u64()
        .or_else(|| error.get("code").and_then(Value::as_u64))
        .unwrap_or(0);
...
fn vue3_model_suite_transform_context(
    options: &Vue3CompilerOptions,
    scope: &Vue3ModelSuiteScope,
) -> Value {
...
fn vue3_core_transform_on_suite_value(payload: &Value) -> Value {
    let source = template_source(payload);
    let options = vue3_options(payload.get("options"));
    let ast = Vue3Dialect::base_parse(source.clone(), &options);
    let mut root = vue3_parse_value(
...
fn vue3_on_suite_transform_children(
    children: Vec<Value>,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ModelSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_on_suite_transform_node(
    mut node: Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ModelSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_on_suite_transform_for_node(
    mut node: Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ModelSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_on_suite_process_dynamic_args(
    node: &mut Value,
    options: &Vue3CompilerOptions,
    scope: &Vue3ModelSuiteScope,
) {
...
fn vue3_on_suite_element_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ModelSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_on_suite_props_codegen(
    node: &Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3ModelSuiteState,
    scope: &Vue3ModelSuiteScope,
...
fn vue3_on_suite_materialize_projection(projection: Option<&Value>, dir: &Value) -> Value {
    let Some(projection) = projection else {
        return Value::Null;
    };
    if projection.is_string() || projection.get("type").is_some() {
...
#[derive(Default)]
struct Vue3SlotOutletSuiteState {
    errors: Vec<Value>,
}

fn vue3_core_transform_slot_outlet_suite_value(payload: &Value) -> Value {
    let source = template_source(payload);
    let options = vue3_options(payload.get("options"));
    let ast = Vue3Dialect::base_parse(source.clone(), &options);
    let mut root = vue3_parse_value(
...
fn vue3_slot_outlet_suite_transform_node(
    mut node: Value,
    options: &Vue3CompilerOptions,
...
fn vue3_slot_outlet_suite_process_directive_expressions(
    node: &mut Value,
    options: &Vue3CompilerOptions,
) {
    if !options.prefix_identifiers {
...
fn vue3_slot_outlet_suite_apply_mutations(node: &mut Value, process: Option<&Value>) {
...
fn vue3_slot_outlet_suite_slot_name(node: &Value, process: Option<&Value>) -> Value {
...
fn vue3_slot_outlet_suite_props_codegen(
    node: &Value,
    indices: &[Value],
    options: &Vue3CompilerOptions,
    state: &mut Vue3SlotOutletSuiteState,
...
fn vue3_slot_outlet_suite_codegen(
    node: &Value,
    slot_name: Value,
    slot_props: Value,
    codegen: &Value,
...
fn vue3_slot_outlet_suite_helpers(root: &Value) -> Vec<String> {
    let mut used = Vec::new();
...
fn vue3_slot_outlet_suite_collect_helpers(node: &Value, used: &mut Vec<&'static str>) {
    if vue3_public_node_type(node) == Some(14)
...
fn vue3_slot_outlet_suite_transform_context(options: &Vue3CompilerOptions) -> Value {
    let mut context = vue3_text_suite_transform_context(options);
    context["scopeId"] = options
        .scope_id
        .clone()
...
#[derive(Default)]
struct Vue3OnceSuiteState {
    cached: usize,
}

fn vue3_core_transform_once_suite_value(payload: &Value) -> Value {
    let source = template_source(payload);
    let options = vue3_options(payload.get("options"));
    let ast = Vue3Dialect::base_parse(source.clone(), &options);
    let mut root = vue3_parse_value(
...
fn vue3_once_suite_transform_children(
    children: Vec<Value>,
    options: &Vue3CompilerOptions,
...
fn vue3_once_suite_transform_node(
    mut node: Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3OnceSuiteState,
    in_v_once: bool,
...
fn vue3_once_suite_transform_if_node(
    siblings: &[Value],
    options: &Vue3CompilerOptions,
    state: &mut Vue3OnceSuiteState,
    in_v_once: bool,
...
fn vue3_once_suite_if_codegen(if_node: &Value) -> Value {
    let branches = if_node
        .get("branches")
        .and_then(Value::as_array)
        .cloned()
...
fn vue3_once_suite_transform_for_node(
    mut node: Value,
    options: &Vue3CompilerOptions,
    state: &mut Vue3OnceSuiteState,
    in_v_once: bool,
...
fn vue3_once_suite_once_projection(node: &Value, in_v_once: bool) -> Value {
...
fn vue3_once_suite_cache_expression(state: &mut Vue3OnceSuiteState, value: Value) -> Value {
    let index = state.cached;
    state.cached += 1;
    json!({
        "type": 20,
...
fn vue3_once_suite_element_codegen(node: &Value, is_block: bool) -> Value {
    match (
        vue3_public_node_type(node),
        node.get("tagType").and_then(Value::as_u64),
    ) {
...
fn vue3_once_suite_props_codegen(node: &Value) -> (Value, Option<Value>, Value) {
    let mut properties = Vec::new();
...
fn vue3_once_suite_object_property(key: Value, value: Value) -> Value {
    json!({
        "type": 16,
        "key": key,
        "value": value,
...
fn vue3_once_suite_simple_expression(content: &str, is_static: bool) -> Value {
    json!({
        "type": 4,
        "content": content,
        "isStatic": is_static,
...
fn vue3_once_suite_quote_string(value: &str) -> String {
    let escaped = value.replace('\\', "\\\\").replace('"', "\\\"");
    format!("\"{escaped}\"")
}

fn vue3_once_suite_component_asset_id(tag: &str) -> String {
...
fn vue3_once_suite_finalize_root(root: &mut Value, state: &Vue3OnceSuiteState) {
    vue3_once_suite_set_root_codegen(root);
    root["components"] = json!(vue3_once_suite_components(root));
...
fn vue3_once_suite_set_root_codegen(root: &mut Value) {
    let children = root
        .get("children")
        .and_then(Value::as_array)
        .cloned()
...
fn vue3_once_suite_helpers(root: &Value) -> Vec<String> {
    let mut used = Vec::new();
...
fn vue3_once_suite_collect_helpers(node: &Value, used: &mut Vec<&'static str>) {
    match vue3_public_node_type(node) {
        Some(13) => {
            if node.get("tag").and_then(Value::as_str) == Some("FRAGMENT") {
                vue3_text_suite_add_helper(used, "FRAGMENT");
...
fn vue3_once_suite_components(root: &Value) -> Vec<String> {
...
fn vue3_text_suite_transform_node(mut node: Value, options: &Vue3CompilerOptions) -> Value {
    if vue3_public_node_type(&node) == Some(1) && vue3_text_suite_directive(&node, "for").is_some()
    {
...
fn vue3_text_suite_transform_for_node(mut node: Value, options: &Vue3CompilerOptions) -> Value {
    let Some(dir) = vue3_text_suite_directive(&node, "for").cloned() else {
        return node;
    };
    let context = vue3_text_suite_transform_context(options);
...
fn vue3_text_suite_materialize_for_parse_result(parse_result: &Value) -> Value {
...
fn vue3_text_suite_process_interpolation(node: &mut Value, options: &Vue3CompilerOptions) {
    if !options.prefix_identifiers {
        return;
    }
...
fn vue3_text_suite_materialize_process_projection(projection: &Value, current: &Value) -> Value {
    match projection.get("kind").and_then(Value::as_str) {
        Some("simple") => json!({
            "type": 4,
            "content": projection.get("content").and_then(Value::as_str).unwrap_or(""),
...
fn vue3_text_suite_materialize_process_child(child: &Value) -> Value {
...
fn vue3_text_suite_compound(children: &[Value]) -> Value {
...
fn vue3_text_suite_finalize_root(root: &mut Value) {
...
fn vue3_text_suite_set_root_codegen(root: &mut Value) {
    let children = root
        .get("children")
        .and_then(Value::as_array)
        .cloned()
...
fn vue3_text_suite_element_codegen(node: &Value, is_block: bool) -> Value {
    if vue3_public_node_type(node) != Some(1)
...
fn vue3_text_suite_for_codegen(node: &Value) -> Value {
...
fn vue3_text_suite_call(callee: &str, arguments: Vec<Value>) -> Value {
    json!({
        "type": 14,
        "callee": callee,
        "arguments": arguments,
...
fn vue3_text_suite_runtime_directives(node: &Value) -> Value {
...
fn vue3_text_suite_collect_directives(root: &Value) -> Vec<String> {
    let mut directives = Vec::new();
    vue3_text_suite_collect_directives_for_node(root, &mut directives);
    directives
}

fn vue3_text_suite_collect_directives_for_node(node: &Value, directives: &mut Vec<String>) {
    for name in vue3_text_suite_runtime_directive_names(node) {
        if !directives.iter().any(|existing| existing == &name) {
            directives.push(name);
        }
...
fn vue3_text_suite_runtime_directive_names(node: &Value) -> Vec<String> {
    node.get("props")
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
...
fn vue3_text_suite_builtin_directive(name: &str) -> bool {
    matches!(
        name,
        "bind"
            | "cloak"
...
fn vue3_text_suite_directive_asset_id(name: &str) -> String {
...
fn vue3_text_suite_helpers(root: &Value) -> Vec<String> {
    let mut used = Vec::new();
...
fn vue3_text_suite_collect_helpers(node: &Value, used: &mut Vec<&'static str>) {
    match vue3_public_node_type(node) {
        Some(5) => vue3_text_suite_add_helper(used, "TO_DISPLAY_STRING"),
        Some(12) => {
            if let Some(codegen) = node.get("codegenNode") {
...
fn vue3_text_suite_add_helper(used: &mut Vec<&'static str>, helper: &'static str) {
    if !used.iter().any(|existing| *existing == helper) {
        used.push(helper);
    }
}

fn vue3_text_suite_single_for_root(root: &Value) -> bool {
    root.get("children")
        .and_then(Value::as_array)
...
fn vue3_text_suite_single_directive_element_root(root: &Value) -> bool {
    root.get("children")
        .and_then(Value::as_array)
        .is_some_and(|children| {
            children.len() == 1
...
fn vue3_text_suite_direct_child_value(child: &Value) -> bool {
    matches!(vue3_public_node_type(child), Some(2 | 5 | 8))
}

fn vue3_text_suite_directive<'a>(node: &'a Value, name: &str) -> Option<&'a Value> {
    node.get("props")
        .and_then(Value::as_array)?
        .iter()
        .find(|prop| {
...
fn vue3_text_suite_remove_directive(node: &mut Value, name: &str) {
    if let Some(props) = node.get_mut("props").and_then(Value::as_array_mut) {
        props.retain(|prop| {
            !(vue3_public_node_type(prop) == Some(7)
                && prop.get("name").and_then(Value::as_str) == Some(name))
...
fn vue3_text_suite_transform_context(options: &Vue3CompilerOptions) -> Value {
    json!({
        "compat": true,
        "ssr": false,
        "inSSR": false,
