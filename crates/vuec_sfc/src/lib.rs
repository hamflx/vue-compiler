...
fn default_script_source_map() -> bool {
    true
}
...
struct Vue3GenericTypeAlias {
    ...
    type_query_declared_types: BTreeMap<String, Vec<String>>,
    define_model_type_query_declared_types: BTreeMap<String, Vec<String>>,
...
fn vue3_global_type_context_from_source(
    source: &str,
    filename: &str,
...
fn collect_vue3_global_types_from_statements(
    source: &str,
    statements: &[Statement<'_>],
...
fn collect_vue3_global_declared_type_from_statement(
    source: &str,
    statement: &Statement<'_>,
    analysis: &mut Vue3ScriptSetupAnalysis,
) {
...
fn collect_vue3_global_type_deps_from_statements(
    statements: &[Statement<'_>],
    analysis: &mut Vue3ScriptSetupAnalysis,
) {
    for statement in statements {
...
fn vue3_statement_is_declare_type(statement: &Statement<'_>) -> bool {
    match statement {
...
fn vue3_declared_type_names_from_statements(statements: &[Statement<'_>]) -> BTreeSet<String> {
    let mut names = BTreeSet::new();
    for statement in statements {
        names.extend(vue3_declared_type_names_from_statement(statement));
    }
...
fn vue3_declared_type_names_from_statement(statement: &Statement<'_>) -> BTreeSet<String> {
    let mut names = BTreeSet::new();
    match statement {
        Statement::TSInterfaceDeclaration(declaration) => {
...
fn vue3_declared_type_names_from_declaration(declaration: &Declaration<'_>) -> BTreeSet<String> {
    let mut names = BTreeSet::new();
    match declaration {
        Declaration::TSInterfaceDeclaration(declaration) => {
            names.insert(declaration.id.name.to_string());
...
fn retain_vue3_type_context_names(context: &mut Vue27TypeContext, names: &BTreeSet<String>) {
    context
        .declared_types
...
fn extend_vue3_type_context_from_external_imports_with_seen(
    filename: &str,
    source: &str,
    source_type: oxc_span::SourceType,
    context: &mut Vue27TypeContext,
...
fn insert_vue3_external_type_alias(
    context: &mut Vue27TypeContext,
    imported: &Vue27TypeContext,
    imported_name: &str,
    local_name: &str,
...
fn insert_vue3_external_generic_alias_string_key_helpers(
    context: &mut Vue27TypeContext,
    imported: &Vue27TypeContext,
    dependency: &str,
) {
...
fn collect_vue3_declared_type_deps_from_statements(
    statements: &[Statement<'_>],
    analysis: &mut Vue3ScriptSetupAnalysis,
) {
    for statement in statements {
...
fn collect_vue3_declared_type_deps_from_statement(
    statement: &Statement<'_>,
    analysis: &mut Vue3ScriptSetupAnalysis,
) {
    match statement {
...
fn collect_vue3_declared_type_deps_from_declaration(
    declaration: &Declaration<'_>,
    analysis: &mut Vue3ScriptSetupAnalysis,
) {
    match declaration {
...
fn collect_vue3_interface_type_deps(
...
fn insert_vue3_declared_type_deps(
    analysis: &mut Vue3ScriptSetupAnalysis,
    name: &str,
...
fn register_vue3_declared_type_query_runtime_types(
    name: &str,
    ty: &TSType<'_>,
    analysis: &mut Vue3ScriptSetupAnalysis,
) {
...
fn vue3_emits_type_from_function(
    source: &str,
...
fn vue3_emits_type_from_literal(
    source: &str,
    literal: &TSTypeLiteral<'_>,
    analysis: &Vue3ScriptSetupAnalysis,
) -> Vue27EmitsType {
    ...
    let mut syntax = Vue3EmitsTypeSyntax::default();
    for member in &literal.members {
        collect_vue3_emits_type_member(source, member, &mut events, &mut syntax, analysis);
    }
    Vue27EmitsType {
        source: source
            .get(literal.span.start as usize..literal.span.end as usize)
...
fn vue3_emits_type_from_interface_body(
    source: &str,
    body: &TSInterfaceBody<'_>,
    analysis: &Vue3ScriptSetupAnalysis,
) -> Vue27EmitsType {
...
fn collect_vue3_emits_from_parameters(
...
fn collect_vue3_emits_from_type(
    ty: &TSType<'_>,
    names: &mut Vec<String>,
    analysis: &Vue3ScriptSetupAnalysis,
) {
...
fn vue3_compile_script_source_map(
    descriptor: &SfcDescriptor,
    generated: &str,
...
fn add_script_block_source_mappings(
    builder: &mut SourceMapBuilder,
    descriptor: &SfcDescriptor,
    block: &SfcBlock,
    generated: &str,
...
fn add_inline_template_source_mappings(
    builder: &mut SourceMapBuilder,
    descriptor: &SfcDescriptor,
...
fn utf16_line_column_for_byte_offset(source: &str, offset: usize) -> Option<(usize, usize)> {
    utf16_zero_based_line_column_for_byte_offset(source, offset)
        .map(|(line, column)| (line + 1, column))
}

fn utf16_zero_based_line_column_for_byte_offset(
    source: &str,
    offset: usize,
) -> Option<(usize, usize)> {
...
fn vue3_compile_script_scope_id(id: Option<&str>) -> Option<String> {
    id.map(|id| {
...
fn vue3_compile_script_short_id(id: Option<&str>) -> String {
    id.and_then(|id| id.strip_prefix("data-v-").or(Some(id)))
        .unwrap_or("")
        .to_string()
}

fn vue3_inline_ssr_css_vars(
    descriptor: &SfcDescriptor,
    options: &SfcScriptCompileOptions,
) -> Option<String> {
...
fn vue3_type_reference_type_argument<'a>(
    reference: &'a TSTypeReference<'a>,
    index: usize,
) -> Option<&'a TSType<'a>> {
    reference
...
fn vue3_resolve_ordered_string_type_keys(
    ty: &TSType<'_>,
    analysis: &Vue3ScriptSetupAnalysis,
) -> Option<Vec<String>> {
    match ty {
...
fn vue3_resolve_template_literal_type_keys(
...
fn vue3_template_type_quasi_value(quasi: &oxc_ast::ast::TemplateElement<'_>) -> String {
    quasi
        .value
        .cooked
        .as_ref()
...
fn vue3_string_mapping_type(name: &str, value: &str) -> String {
    match name {
...
fn vue3_literal_type_key(literal: &TSLiteral<'_>) -> Option<String> {
    match literal {
        TSLiteral::StringLiteral(literal) => Some(literal.value.to_string()),
        TSLiteral::NumericLiteral(literal) => Some(literal.value.to_string()),
        TSLiteral::BigIntLiteral(literal) => Some(literal.value.to_string()),
...
fn record_vue3_type_argument_deps(
    ...
    analysis
        .deps
        .extend(collect_vue3_type_argument_deps(type_argument, analysis));
}

fn collect_vue3_type_argument_deps(
    type_argument: &TSType<'_>,
    analysis: &Vue3ScriptSetupAnalysis,
) -> BTreeSet<String> {
    let mut deps = BTreeSet::new();
...
fn collect_vue3_type_argument_deps_into(
    type_argument: &TSType<'_>,
    analysis: &Vue3ScriptSetupAnalysis,
...
fn collect_vue3_signature_type_deps(
...
fn collect_vue3_formal_parameters_type_deps(
...
fn collect_vue3_type_annotation_deps(
...
fn collect_vue3_tuple_element_type_deps(
...
fn vue3_type_members_from_mapped_type(
    source: &str,
...
fn vue3_type_members_from_interface(
    source: &str,
    declaration: &TSInterfaceDeclaration<'_>,
    analysis: &Vue3ScriptSetupAnalysis,
) -> Vue27TypeMembers {
...
fn vue3_emits_type_from_interface(
    source: &str,
    declaration: &TSInterfaceDeclaration<'_>,
    analysis: &Vue3ScriptSetupAnalysis,
) -> Vue27EmitsType {
...
fn vue3_interface_heritage_type_source(
    source: &str,
    heritage: &TSInterfaceHeritage<'_>,
) -> Option<String> {
...
fn vue3_expression_type_name_key(expression: &Expression<'_>) -> Option<String> {
...
fn vue3_resolve_interface_heritage_keyof_runtime_type(
    source: &str,
    heritage: &TSInterfaceHeritage<'_>,
    analysis: &Vue3ScriptSetupAnalysis,
) -> Option<Vec<String>> {
...
fn vue3_keyof_runtime_type_from_signatures(
    signatures: &[TSSignature<'_>],
    analysis: &Vue3ScriptSetupAnalysis,
) -> Option<Vec<String>> {
    let mut types = Vec::new();
...
fn infer_vue3_keyof_runtime_type(
    node: &TSType<'_>,
    analysis: &Vue3ScriptSetupAnalysis,
) -> Option<Vec<String>> {
    match node {
